...
# of its consecutive elements for being a zigzag. More formally, your task is to
# construct an array of length numbers.length - 2, where the ith element of the
# output array equals 1 if the triple (numbers[i], numbers[i + 1], numbers[i + 2]) is a zigzag,
# and 0 otherwise.
#
...
def solution(numbers):
    ...
    while c < len(numbers):
        print(c)
        if numbers[a] < numbers[b] > numbers[c] or numbers[a] > numbers[b] < numbers[c]:
            solution.append(1)
        else:
            solution.append(0)
        a+=1
